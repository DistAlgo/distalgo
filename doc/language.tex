...
\newcommand{\exerciselist}[1]{#1} %
\newcommand{\exercise}[1]{ %
  \refstepcounter{exercounter} \paragraph{Exercise \theexercounter} #1}
\newcommand{\exername}[1]{$\!\!\!\!$\b{(#1)}\,} %
\newcommand{\sol}[1]{} %
...
\newcommand\defnn[2]{{\it #1}\index{#2|bold}} %
\newcommand\defna[2]{{\it #1}\index{#2@#1|bold}} %
\newcommand\defnu[2]{{\it #1}\index{#2!#1|bold}} %
\newcommand\term[1]{#1\index{#1}}
\newcommand\termm[2]{#1\index{#2}}
\newcommand\terma[2]{#1\index{#2@#1}} %
\newcommand\termu[2]{#1\index{#2!#1}} %
\newcommand\people[1]{#1\index{#1}}
...
\newcommand\cm[1]{\m{\c{#1}}} %
\renewcommand\b[1]{{\bf #1}} %
\newcommand\power{\texttt{\hspace{0pt}\char`\^\hspace{0pt}}}
\newcommand{\ctrue}{\c{true}\xspace}
\newcommand{\cfalse}{\c{false}\xspace}
\newcommand{\cwhile}{\c{while}\xspace}
\newcommand{\cfor}{\c{for}\xspace}
\newcommand{\cforall}{\c{all}} %
\newcommand{\cthis}{\c{self}\xspace} %
\newcommand{\cif}{\c{\hspace{-1pt}if~}} %
\newcommand{\celse}{\c{else}} %
\newcommand{\cset}{\c{set}} %
\newcommand{\cempty}{\c{new~set}} %
\newcommand{\cnot}{\c{\hspace{4pt}not\hspace{5pt}}} %
\newcommand{\cin}{\c{\hspace{4pt}in\hspace{5pt}}} %
\newcommand{\cadd}{\c{add}} %
\newcommand{\cdel}{\c{del}} %
\newcommand{\ccontains}{\c{contains}} %
\newcommand{\elem}{\c{members}} %
\newcommand{\cclass}{\c{class~}} %
\newcommand{\DEF}{\b{def}~} %
\newcommand{\THEN}{\b{then}~} %
\newcommand{\ELSE}{\b{else}~} %
\newcommand{\LET}{\b{let}~} %
\newcommand{\IN}{\b{in}~} %
\newcommand\p[1]{{\small\mathify{\it #1}}}
\newcommand{\cmd}{{\it cmd}} %
...
\newenvironment{invl}{\begin{center}}{\end{center}}
\newenvironment{invln}{\begin{equation}}{\end{equation}\ignorespacesafterend}

\renewcommand\O[1]{\mathify{O(#1)}\xspace}
\newcommand\s[1]{\mathify{\#}\c{#1}}
\renewcommand\arg[2]{\mathify{\c{#1}.#2}}
\newcommand\relarg[3]{\mathify{\#\c{#1}.#2/#3}}
\newcommand\fire{{\it firedTimes}\xspace}
\newcommand\I[1]{\mbox{{\small \it I}\c{#1}}}
\newcommand\D[1]{\mbox{{\small \it D}\c{#1}}}
\newcommand{\atime}{{\it time}} %
\newcommand{\afreq}{{\it freq}} %

\newenvironment{twoLineFigure}[1]
  {\begin{figure*}[#1]
      \rule{\textwidth}{.1mm}}{\rule{\textwidth}{.1mm}
  \end{figure*}\ignorespacesafterend}

\newcommand{\Vspace}[1]{\rule{0cm}{#1ex}}

\newcommand{\INV}{\b{\hspace{-.9ex}inv~\hspace{.1ex}}}
\newcommand{\query}{{\it query}\xspace} %
\newcommand{\AT}{\b{at~\hspace{.2ex}}}
\newcommand{\update}{{\it update}\xspace} %
\newcommand{\IF}{\b{if}~} %
\newcommand{\cond}{{\it condition}}
\newcommand{\DE}{\b{de~\hspace{1pt}}}
\newcommand{\class}{{\it class}}
\newcommand{\decl}{{\it declaration}}
\newcommand{\DO}{\b{do}~} %
\newcommand{\BEFORE}{\b{before~}}
\newcommand{\AFTER}{\b{after~}}
\newcommand{\INSTEAD}{\b{instead~}}
\newcommand{\maint}{{\it maint}\xspace}
\newcommand{\COST}{\b{cost~}}
\newcommand{\cost}{{\it cost}\xspace}
\newcommand{\vquery}{{\it query}} %
\newcommand{\vupdate}{{\it update}} %
\newcommand{\vqos}{{\it qos}} %
\newcommand{\vaddqo}{{\it addqo}} %
\newcommand{\velemmod}{{\it elementmodified}} %

\newcommand{\aread}{{\rm read}}
\newcommand{\awrite}{{\rm write}}
\newcommand{\amayalias}{{\rm mayalias}}
\newcommand{\atype}{{\rm type}}
\newcommand{\avars}{{\rm vars}}
\newcommand{\aclass}{{\rm class}}
\newcommand{\amethod}{{\rm method}}
\newcommand{\aisfield}{{\rm isfield}}
\newcommand{\aisfieldof}{{\rm isfieldof}}
\newcommand{\aisupdatedin}{{\rm isupdatedin}}
...
initially setting up data in the process before the execution starts.
...
must be defined in \c{\p{process\_body}} for carrying out the main flow of
execution.
...
The number \p{n} and clause \c{at} are optional; the defaults are 1 and
local node, respectively.  When both the number \p{n} and clause \c{at} are
omitted, a single process is created and assigned to \p{v}; otherwise, a
set of processes is created and assigned to \p{v}.
\begin{code}
...
the program as a place where control may yield to handling of received
messages.
\begin{code}
    -- \p{l}:
\end{code}
...
A handler definition is of the following form. 
...
\begin{code}
    receive \p{mexp} from \p{pexp} at \p{l\sb{1}}, ..., \p{l\sb{j}}:
      \p{handler_body}
\end{code}
We could use the noun \c{handler} in place of \c{receive}, but handlers are
not named and called with their names; instead, yield points are named, and
handlers are executed at the specified yield points.
...
un-handled messeges, by adding un-handled messages before any matching
...
value of Boolean-valued expression \p{bexp} to become true, with an
implicit yield point for handling messages while waiting.  It is a short
hand for \c{await \p{bexp}:~pass} in a general, nondeterministic \c{await}
statement.
\begin{code}
    await \p{bexp}
\end{code}
...
\p{bexp\sb{k}} to become true or a timeout after \p{t} seconds, with an
implicit yield point for handling messages while waiting, and then
nondeterministically selects one\notes{} of statements
...
An \c{await} statement must be preceded by a yield point; if a yield point
is not specified explicitly, the default is that all message handlers can
...
for all values of variables that satisfy all membership clauses
...
\c{x}; it is equivalent to using a fresh variable \c{y} instead and adding
a conjunct \c{y=x} in condition \p{bexp}.
...
\c{min}, and \c{max}.
The query returns the value of applying \c{\p{agg}} to the set value of the
comprehension expression \c{\p{comprehension\_exp}}, for the first form, or
to the multiset of values of \p{exp} for all values of variables that
satisfy all membership clauses \c{\p{v\sb{i}} in \p{sexp\sb{i}}} and
condition \p{bexp}, for the second form.
\begin{code}
    \p{agg} \p{comprehension_exp}
...
A quantification is a query of one of the following two forms plus a set of
parameters.  The two forms are called existential and universal
quantifications, respectively.  Given values of parameters, the query
returns \c{true} iff for some or all, respectively, values of the variables
that satisfy all membership clauses \c{\p{v\sb{i}} in \p{sexp\sb{i}}},
expression \p{bexp} evaluates to \c{true}.  When an existential
quantification returns \c{true}, all variables in the query are also bound
to a combination of values, called a witness, that satisfy all the
membership clauses and condition \p{bexp}.
\begin{code}
...
\end{code}
Parameters are indicated as for comprehensions.  Also as for
comprehensions, when \p{sexp_i} is a variable \p{s_i}, expression
\p{s_i(v_i)} can be used in place of \c{\p{v_i} in \p{s_i}}.  When \p{bexp}
is \c{true}, the \c{has} clause can be omitted.
...
In the clauses \c{\p{v\sb{1}} in \p{sexp\sb{1}}, \ldots, \p{v\sb{k}} in
  \p{sexp\sb{k}}} in all of comprehensions, aggregations, and
quantifications, a tuple expression \p{texp_i}, called a tuple pattern, may
occur in place of variable \p{v_i}.  Previously unbound variables in
\p{texp_i} are bound to the corresponding components in %
...
The following statement configures the system to handle all messages
received at each yield point; this is the default. 
